...
/** Google's mark, matching the badge the shared shell puts on the picture. */
function GoogleMark() {
  return (
...
function SessionPanel({ session }: { session: ForgeSession }) {
  return (
    <div className="flex flex-col gap-2 border-t border-border pt-3">
      ...
      <IdentitySync name={session.name} email={session.email} image={session.image} />
      <div className="flex min-w-0 items-center gap-2.5 px-1">
        <span className="relative flex h-8 w-8 flex-none" aria-hidden="true">
          ...
          <span className="absolute -bottom-px -right-px flex h-[15px] w-[15px] items-center justify-center rounded-full bg-surface-strong p-[2px] ring-1 ring-border">
            <GoogleMark />
          </span>
        </span>
